...
import { useSedesLite, searchSedesForSelect, getSedeByIdForSelect } from '../../sedes';
import { 
  FormWrapper, 
  FormWrapperInput, 
  FormWrapperSelect, 
...
import { FormWrapperSelectHttp } from '../../../../../common/components/FormWrapperSelectHttp';
...
export const LigaForm: React.FC<LigaFormProps> = ({
  ...
  const [initialSedeValues, setInitialSedeValues] = useState<any[]>();
  ...
  useEffect(() => {
    ...
    // Crear objeto sede si hay datos iniciales
    if (initialData?.sedeId && initialData?.sede) {
      setInitialSedeValues([{
        id: initialData.sedeId,
        nombre: initialData.sede.nombre || 'Sede seleccionada',
        direccion: initialData.sede.direccion || '',
        numeroCancha: 1 // Valor por defecto ya que no está en el tipo Liga.sede
      }]);
    }
  }, [initialData]);
            ...
            <div className="border-t border-gray-200 dark:border-gray-600 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {Object.entries(CriteriosDesempateEnum).map(([key, value]) => {
                  ...
                  const isSelected = selectedCriterios.includes(value);
                  const orderIndex = isSelected ? selectedCriterios.indexOf(value) + 1 : null;
                  ...
                  return (
                    <label key={value} className="flex items-center space-x-3 p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                      ...
                      <span className="text-sm font-medium text-gray-900 dark:text-white">
                        {isSelected ? `${orderIndex} - ${labels[key] || key}` : labels[key] || key}
                      </span>
                    </label>
                  );
                })}
            ...
            <div className="border-t border-gray-200 dark:border-gray-600 pt-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormWrapperSelectHttp
                  ...
                  searchFunction={searchSedesForSelect}
                  getByIdFunction={getSedeByIdForSelect}
                  values={initialSedeValues}
                  valueKey="id"
                  displayKey="nombre"
